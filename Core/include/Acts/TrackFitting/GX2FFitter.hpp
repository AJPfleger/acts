...
struct GX2FFitterResult {
  ...
  bool smoothed = false;  /// remove

  // Indicator if navigation direction has been reversed
  bool reversed = false;  /// remove
...
class GX2FFitter {
 ...
 private:
  ...
  class AborterKF {
   public:
    /// Broadcast the result_type
    using action_type = ActorKF<parameters_t>;
 ...
 public:
  ...
  auto fit(source_link_iterator_t it, source_link_iterator_t end,
    ...
    /// Understand Aborter, Actor, Result later
    /// Aborter can be reused? we would need it as part of propagator only
    // Create the ActionList and AbortList
    using KalmanAborter = AborterKF<parameters_t>;
    using KalmanActor = ActorKF<parameters_t>;

    using KalmanResult = typename KalmanActor::result_type;
    using Actors = ActionList<KalmanActor>;
    using Aborters = AbortList<KalmanAborter>;

    /// What are Propagator options?
    ...
    const size_t nUpdateMax = 1;  // set higher or as an input parameter
    for (size_t nUpdate = 0; nUpdate < nUpdateMax; nUpdate++) {
      /// update params
