...
std::tuple<ActsScalar, ActsScalar, bool> mergedPhiSector(
  ...
  if (std::abs(hlPhi1 - std::numbers::pi_v<ActsScalar> / 2.) < tolerance &&
      std::abs(hlPhi2 - std::numbers::pi_v<ActsScalar> / 2.) < tolerance) {
    ACTS_VERBOSE("Both phi sectors cover a half circle");

    ACTS_VERBOSE("-> distance between sectors: "
                 << detail::difference_periodic(
                        avgPhi1, avgPhi2, 2 * std::numbers::pi_v<ActsScalar>) /
                        1_degree);
    ...
    ActsScalar newAvgPhi =
        detail::radian_sym(avgPhi1 + std::numbers::pi_v<ActsScalar> / 2.);
