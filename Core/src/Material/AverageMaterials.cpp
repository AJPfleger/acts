...
Acts::MaterialSlab Acts::detail::combineSlabs(const MaterialSlab& slab1,
  ...
  const double molarDensity1 = mat1.molarDensity();
  const double molarDensity2 = mat2.molarDensity();
  ...
  const double z1 = mat1.Z();
  const double z2 = mat2.Z();
  ...
  if (z1 > 0. && z2 > 0. && thicknessWeight1 > 0. && thicknessWeight2 > 0.) {
    z = std::exp(thicknessWeight1 * std::log(z1) +
                 thicknessWeight2 * std::log(z2));
  } else {
    z = thicknessWeight1 * z1 + thicknessWeight2 * z2;
  }
