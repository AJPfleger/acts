...
TrapezoidVolumeBounds::TrapezoidVolumeBounds(ActsScalar minhalex,
  ...
  m_values[eBeta] = std::numbers::pi_v<ActsScalar> - get(eAlpha);
  checkConsistency();
  buildSurfaceBounds();
}
...
TrapezoidVolumeBounds::TrapezoidVolumeBounds(ActsScalar minhalex,
  ...
  ActsScalar gamma = (alpha > beta)
                         ? (alpha - 0.5 * std::numbers::pi_v<ActsScalar>)
                         : (beta - 0.5 * std::numbers::pi_v<ActsScalar>);
...
std::vector<OrientedSurface> TrapezoidVolumeBounds::orientedSurfaces(
  ...
  auto fbTransform =
      ...
      AngleAxis3(-0.5 * std::numbers::pi_v<ActsScalar> + get(eBeta),
                 Vector3(0., 0., 1.)) *
      s_planeYZ;
  sf =
  ...
  auto faTransform =
      ...
      AngleAxis3(-0.5 * std::numbers::pi_v<ActsScalar> + get(eAlpha),
                 Vector3(0., 0., 1.)) *
      s_planeYZ;
...
void TrapezoidVolumeBounds::buildSurfaceBounds() {
  ...
  m_faceAlphaRectangleBounds = std::make_shared<const RectangleBounds>(
      get(eHalfLengthY) /
          cos(get(eAlpha) - 0.5 * std::numbers::pi_v<ActsScalar>),
      get(eHalfLengthZ));

  m_faceBetaRectangleBounds = std::make_shared<const RectangleBounds>(
      get(eHalfLengthY) /
          cos(get(eBeta) - 0.5 * std::numbers::pi_v<ActsScalar>),
      get(eHalfLengthZ));
