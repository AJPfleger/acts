...
BOOST_AUTO_TEST_CASE(CylinderVolumeBoundsSetValues) {
  ...
  BOOST_CHECK_THROW(cyl.set(CylinderVolumeBounds::eHalfPhiSector,
  ...
  BOOST_CHECK_THROW(cyl.set(CylinderVolumeBounds::eHalfPhiSector,
                            1.5 * std::numbers::pi_v<ActsScalar>),
                    std::invalid_argument);
  BOOST_CHECK_EQUAL(cyl.get(CylinderVolumeBounds::eHalfPhiSector),
                    std::numbers::pi_v<ActsScalar>);

  cyl.set(CylinderVolumeBounds::eHalfPhiSector,
          std::numbers::pi_v<ActsScalar> / 2.);
  BOOST_CHECK_EQUAL(cyl.get(CylinderVolumeBounds::eHalfPhiSector),
                    std::numbers::pi_v<ActsScalar> / 2.);
  ...
  for (auto bValue :
    ...
    BOOST_CHECK_THROW(cyl.set(bValue, -1.5 * std::numbers::pi_v<ActsScalar>),
                      std::invalid_argument);
    ...
    cyl.set(bValue, 0.5 * std::numbers::pi_v<ActsScalar>);
    BOOST_CHECK_EQUAL(cyl.get(bValue), 0.5 * std::numbers::pi_v<ActsScalar>);
    cyl.set(bValue, -0.5 * std::numbers::pi_v<ActsScalar>);
    BOOST_CHECK_EQUAL(cyl.get(bValue), -0.5 * std::numbers::pi_v<ActsScalar>);
  }
