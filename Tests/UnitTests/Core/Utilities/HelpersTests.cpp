...
BOOST_AUTO_TEST_CASE(incidentAnglesTest) {
  ...
  // 45 degree on both axes
  {
    ...
    CHECK_CLOSE_ABS(a0, std::numbers::pi_v<ActsScalar> / 4.,
                    std::numeric_limits<ActsScalar>::epsilon());
    CHECK_CLOSE_ABS(a1, std::numbers::pi_v<ActsScalar> / 4.,
                    std::numeric_limits<ActsScalar>::epsilon());
  }
  ...
  // 45 degree on first axis
  {
    ...
    CHECK_CLOSE_ABS(a0, std::numbers::pi_v<ActsScalar> / 4.,
                    std::numeric_limits<ActsScalar>::epsilon());
    CHECK_CLOSE_ABS(a1, std::numbers::pi_v<ActsScalar> / 2.,
                    std::numeric_limits<ActsScalar>::epsilon());
  }
  ...
  // 45 degree on second axis
  {
    ...
    auto [a0, a1] = incidentAngles(dir, ref);
    CHECK_CLOSE_ABS(a0, std::numbers::pi_v<ActsScalar> / 2.,
                    std::numeric_limits<ActsScalar>::epsilon());
    CHECK_CLOSE_ABS(a1, std::numbers::pi_v<ActsScalar> / 4.,
                    std::numeric_limits<ActsScalar>::epsilon());
  }
  ...
  // 45 degree but different quadrant
  {
    Vector3 dir = {-1, -1, 1};
    auto [a0, a1] = incidentAngles(dir, ref);
    CHECK_CLOSE_ABS(a0, 3 * std::numbers::pi_v<ActsScalar> / 4.,
                    std::numeric_limits<ActsScalar>::epsilon());
    CHECK_CLOSE_ABS(a1, 3 * std::numbers::pi_v<ActsScalar> / 4.,
                    std::numeric_limits<ActsScalar>::epsilon());
  }
