...
mode=${1:-all}
if ! [[ $mode = @(all|kf|gsf|gx2f|refit_kf|refit_gsf|refit_gx2f|fullchains|simulation) ]]; then
    echo "Usage: $0 <all|kf|gsf|gx2f|refit_kf|refit_gsf|refit_gx2f|fullchains|simulation> (outdir)"
    exit 1
fi
...
if [[ "$mode" == "all" || "$mode" == "refit_gx2f" ]]; then
    run_physmon_gen "Truth Tracking GX2F refit" "trackrefitting_gx2f"
fi
